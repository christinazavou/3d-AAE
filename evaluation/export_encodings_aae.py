...
def main(eval_config):
    ...
    train_results_path = join(eval_config['results_root'], eval_config['arch'], eval_config['experiment_name'])
    ...
    if eval_config['dataset'].lower() == 'buildingcomponentdataset2':
        from datasets.buildingcomponent import BuildingComponentDataset2
        test_dataset = BuildingComponentDataset2(txt_file=eval_config['test_txt'],
                                                n_points=eval_config['n_points'],
                                                data_root=eval_config['test_data_root'])
        if "train_dataset" in eval_config:
            train_dataset = BuildingComponentDataset2(txt_file=eval_config['train_txt'],
                                                      n_points=eval_config['n_points'],
                                                      data_root=eval_config['train_data_root'])
    else:
